...
const myScripts = scripts[currentNetwork] as any;
const mySystemScripts = systemScripts[currentNetwork] as any;
...
export async function unlock(
  ...
  const newWitnessArgs = new ccc.WitnessArgs(
    stringToBytesHex(preimageForWitness) as `0x${string}`,
  );
